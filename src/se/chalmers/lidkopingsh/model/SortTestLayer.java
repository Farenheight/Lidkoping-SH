...
/**
 * Creates a dummy model mainly for sort tests. Containing orders with
 * tasks by the following pattern:
 * 
 * Order TASK_COUNT:      TASK_COUNT tasks done 
 * Order TASK_COUNT + 1:  0 tasks done
 * Order TASK_COUNT + 2:  1 tasks done 
 * Order TASK_COUNT + n:  n - 1 tasks done
 * 
 * Ex. TASK_COUNT == 2
 * 
 * Order 0: 0 task done 
 * Order 1: 1 task done 
 * Order 2: 2 tasks done 
 * Order 3: 0 tasks done
 * Order 4: 1 tasks done 
 * etc
 * 
 * @author Simon
 * 
	...
	private Order generateOrder(List<Station> stations) {
		...
		Order order = new Order(count, "1200", "O."
				...
				new Customer("Mr", "Olle Bengtsson", "Testvagen 52",
						...
						(int) System.currentTimeMillis()), pList);
		pList.add(new Stone());
		++count;
